...
class BaseDataLoader(DataLoader):
    ...
    def __init__(self, dataset, patient_tags, oversampling_factor, undersampling_factor, random,
                 random_state_data_loader, batch_size, shuffle
        ...
        self.sampler, self.valid_sampler, self.test_sampler, \
            self.train_idx, self.valid_idx, self.test_idx, self.n_samples = self._split_sampler(oversampling,
                                                                                                undersampling)
    ...
    def _split_sampler(self, oversampling=True, undersampling=False):
        ...
        if oversampling:
            ...
            select_patient_index = [idx for idx in self.rng.choice(len(train_positive)
                                                                   , size=int(
                    len(train_negative) * self.oversampling_factor), replace=replace)]
        ...
        if undersampling:
            ...
            select_patient_index = [idx for idx in self.rng.choice(len(train_negative),
                                                                   size=int(
                                                                       len(train_negative) * self.undersampling_factor),
                                                                   replace=replace)]
